...
export default function SmartMedia({
  ...
  // --- PDF RENDERING (Protected Embedded Viewer) ---
  if (isPdf) {
    if (isFullView || controls) {
      return (
        <div
          ...
          className={`flex flex-col w-full h-full select-none ${fill ? "absolute inset-0" : "min-h-[480px]"}`}
        >
          <div className="relative flex-1 w-full h-full min-h-[420px] rounded-xl overflow-hidden bg-slate-950 border border-white/15">
            <iframe
              src={`${src}#toolbar=0&navpanes=0&scrollbar=0&view=FitH`}
              title={alt}
              className="w-full h-full min-h-[420px] border-0 pointer-events-auto"
            />
          </div>
    ...
    return (
      <div
        onClick={onClick}
        className={`flex flex-col items-center justify-center p-3 text-center bg-gradient-to-br from-rose-950/40 via-slate-900/60 to-slate-950/80 border border-rose-500/20 ${
  ...
  // --- VIDEO RENDERING (Protected Player) ---
  if (isVideo) {
    return (
      <div
        onClick={onClick}
